...
public class MainController {
    ...
    public ResponseEntity<Void> deleteTrack(@PathVariable("id") long id) {
        ...
        if(existingTrack.isPresent()) {
            ...
            for (Room room : existingTrack.get().getRooms()) {
                room.removeTrack(existingTrack.get());
                roomRepo.save(room);
            }
    ...
    public ResponseEntity<Void> addRoom (String roomName, Boolean roomState) {
        Room room = new Room(roomName, roomState);
        roomRepo.save(room);
        URI uri = WebMvcLinkBuilder.linkTo(MainController.class).slash("room").slash(room.getId()).toUri();
        return ResponseEntity.created(uri).build();
    }
    ...
    public ResponseEntity<Void> removeRoom(@PathVariable("id") long id) {
        ...
        if (existingRoom.isPresent()) {
            for (Track track : existingRoom.get().getTrackSet()) {
                track.getRooms().remove(existingRoom.get());
                trackRepository.save(track);
            }
    ...
    @GetMapping(path="/tracks/{id}/rooms")
    public ResponseEntity<Set<Room>> getTrackRooms(@PathVariable("id") long id) {
        Optional<Track> existingTrack = trackRepository.findById(id);
        if(existingTrack.isPresent()) {
            Set<Room> rooms = existingTrack.get().getRooms();
            if (!rooms.isEmpty()) {
                return ResponseEntity.ok(rooms);
            } else {
                return ResponseEntity.noContent().build();
    ...
    public ResponseEntity<Set<Track>> getRoomsTracks(@PathVariable("roomId") long roomId) {
        Optional<Room> room = roomRepo.findById(roomId);
        if (room.isPresent()) {
            Set<Track> tracks = room.get().getTrackSet();
            if(!tracks.isEmpty()) {
                return ResponseEntity.ok(tracks);
            } else {
                return ResponseEntity.noContent().build();
